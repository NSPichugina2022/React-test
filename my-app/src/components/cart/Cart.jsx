...
const Cart = (props) => {
    ...
    const onAddOverlay = async (obj)=>{
        try{
            ...
            if(findOverlay)
            {
            axios.delete(`https://638c8265eafd555746a7d670.mockapi.io/cart/${findOverlay.id}`)
            props.setOverlayItems((over) => over.filter(item => item.myId !== obj.myId))
            }
            else{
              const {data} = await axios.post('https://638c8265eafd555746a7d670.mockapi.io/cart', obj)
    ...
    const onAddFav = async (obj)=>{
        ...
        try{
            const findFavorites = props.favorites.find(objFav => objFav.myId === obj.myId)
             if(findFavorites){
            axios.delete(`https://638c8265eafd555746a7d670.mockapi.io/favorites/${findFavorites.id}`)
            props.setFavorites((over) => over.filter(item => item.myId !== obj.myId))

        }
        else{
          const {data} = await  axios.post('https://638c8265eafd555746a7d670.mockapi.io/favorites', obj)
